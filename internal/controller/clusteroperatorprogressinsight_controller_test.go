...
func Test_allClusterOperators(t *testing.T) {
	...
	s := runtime.NewScheme()
	err := openshiftconfigv1.AddToScheme(s)
	if err != nil {
...
func Test_cvVersion(t *testing.T) {
	testCases := []struct {
		name     string
		objName  string
		expected bool
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			result := cvVersion.Create(event.CreateEvent{Object: obj})
			if result != tc.expected {
				t.Errorf("cvVersion.Create() = %v, expected %v", result, tc.expected)
			}

			result = cvVersion.Update(event.UpdateEvent{ObjectOld: obj, ObjectNew: obj})
			if result != tc.expected {
				t.Errorf("cvVersion.Update() = %v, expected %v", result, tc.expected)
			}

			result = cvVersion.Delete(event.DeleteEvent{Object: obj})
			if result != tc.expected {
				t.Errorf("cvVersion.Delete() = %v, expected %v", result, tc.expected)
			}

			result = cvVersion.Generic(event.GenericEvent{Object: obj})
			if result != tc.expected {
