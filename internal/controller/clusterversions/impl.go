...
func (r *Reconciler) reconcileHealthInsights(ctx context.Context, cvProgressInsight *ouev1alpha1.ClusterVersionProgressInsight, healthInsights []*ouev1alpha1.UpdateHealthInsightStatus) error {
	var clusterInsights ouev1alpha1.UpdateHealthInsightList
	if err := r.List(ctx, &clusterInsights, client.MatchingLabels{health.InsightManagerLabel: "clusterversion"}); err != nil {
	...
	for _, insight := range toCreate.UnsortedList() {
		healthInsight := &ouev1alpha1.UpdateHealthInsight{
			ObjectMeta: metav1.ObjectMeta{
				Name:   insight,
				Labels: map[string]string{health.InsightManagerLabel: "clusterversion"},
			},
