...
func assessNode(insight ouev1alpha1.NodeProgressInsight) nodeDisplayData {
	...
	var updating *metav1.Condition
	if updating = meta.FindStatusCondition(insight.Status.Conditions, string(ouev1alpha1.NodeStatusInsightUpdating)); updating != nil {
		switch {
	...
	if degraded := meta.FindStatusCondition(insight.Status.Conditions, string(ouev1alpha1.NodeStatusInsightDegraded)); degraded != nil {
		if node.isDegraded = degraded.Status == metav1.ConditionTrue; node.isDegraded {
			node.Assessment = nodeAssessmentDegraded
		}
	}

	if available := meta.FindStatusCondition(insight.Status.Conditions, string(ouev1alpha1.NodeStatusInsightAvailable)); available != nil {
